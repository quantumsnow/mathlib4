...
noncomputable def doubleSlicePair : TopCat.{u} ⥤ TopPair.{u} where
  obj X := TopPair.of (A := X ⨿ X) (X := cylinder.obj X) (coprod.desc ((cylinder.sliceIncl 0).app X) ((cylinder.sliceIncl 1).app X)) sorry
  map f := TopPair.ofHom (cylinder.map f) (coprod.map f f) sorry

end cylinder

/-- The categorical quotient of of a topological space. Note that this is not always equal to the topological quotient via `Quotient`. The two will be isomorphic when the collapsed space is nonempty but whereas `Quotient ⊥` (the quotient by nothing) of a space `X` is isomorpic to `X`, in this categorical definition, `X/∅ ≅ X ⨿ ∗`. -/
noncomputable def quotient : TopPair.{u} ⥤ TopCat.Pointed.{u} where
  ...
  map {Xₚ Yₚ} f := ⟨pushout.map Xₚ.map _ Yₚ.map _ (TopPair.Hom.fst f) (isTerminalPUnit.from _) (TopPair.Hom.snd f) (by cat_disch) (by cat_disch), sorry⟩
...
noncomputable def quotient.inclIsoAdjoinPoint : TopPair.incl ⋙ TopCat.quotient ≅ adjoinPoint := sorry -- TODO: data

/-- The cone of a topological space `X` is the space `CX := (X × I)/(X × {0})` with basepoint the class `[X × {0}]`. -/
...
/-- The suspension of a topological space `X` is the space `SX := (X × I)/(X × {0})/(X × {1})` with basepoint `[X × {1}]`. -/
noncomputable def suspension : TopCat.{u} ⥤ Pointed.{u} where
  obj X := ⟨pushout (cylinder.doubleSlicePair.obj X).hom (coprod.map (isTerminalPUnit.from X) (isTerminalPUnit.from X)), pushout.inr (cylinder.doubleSlicePair.obj _).hom _ (coprod.inr (C := TopCat) PUnit.unit)⟩
...
/-- The mapping cone of a map `f : Y ⟶ X` is the gluing of the cone `CY` to `X` along `f`. We will often denote this by `X ∪ CY`. -/
noncomputable def mappingCone : Arrow TopCat.{u} ⥤ TopCat.{u} where
  obj a := pushout a.hom (cone.incl.app a.left)
  map {a _} f := pushout.map a.hom _ _ _ f.right (cone.map f.left).toTopCatHom f.left (by cat_disch) sorry
...
def conePairQuotientIso : MorphismProperty.Arrow.forget _ _ _ ⋙ conePair ⋙ quotient ≅ quotient := sorry -- TODO: data

end mappingCone

namespace Pointed

/-- The pointed cylinder of a topological space `X` with basepoint`x₀` is `X × I`, with basepoint `(x₀, i)`. -/
...
/-- The inclusion of `I ≅ I × {x₀}` with basepoint `i` into the cylinder of `X` with basepoint `x₀`. -/
...
/-- The cone of a topological space `X` with basepoint `x₀` is the space `CX := (X × I)/(X × {0})` with basepoint `(x₀, 1)`. -/
...
a precomposition because it makes `Sⁿ⁺¹ = SSⁿ` a definitional equality which tends to be more useful. -/
noncomputable def suspension' (n : ℕ) : Pointed.{u} ⥤ Pointed.{u} := (forget ⋙ suspension).iteratePostcomp n
...
/-- The inclusion of `I ≅ I × {x₀}` with basepoint `i` into the suspension of `X` with basepoint `x₀`. -/
noncomputable def intervalIncl : (Functor.const TopCat.Pointed.{u}).obj (I 1) ⟶ TopCat.Pointed.forget ⋙ suspension where
...
noncomputable def forgetIso : (n : ℕ) → TopCat.Pointed.forget ⋙ TopCat.suspension' n ≅
  ...
  | 0 => Iso.refl _
  | n + 1 => (Functor.associator _ _ _).symm ≪≫ Functor.isoWhiskerRight (forgetIso n) _ ≪≫ Functor.associator _ _ _ ≪≫ Functor.isoWhiskerLeft _ (Functor.associator _ _ _).symm ≪≫ (Functor.associator _ _ _).symm

/-- The reduced suspension of a space pointed `X` is `ΣX := SX/I`. -/
noncomputable def reduced : Pointed.{u} ⥤ Pointed.{u} := intervalPair ⋙ TopPair.Pointed.forget ⋙ quotient
...
noncomputable abbrev toReduced : TopCat.Pointed.forget ⋙ suspension ⟶ reduced := intervalPair.whiskerLeft quotient.out
...
/-- The mapping cone of a map `f : Y ⟶ X` is the gluing of the cone of `Y` to `X` along `f`. We will often denote this by `X ∪ CY`. -/
noncomputable def mappingCone : Arrow TopCat.Pointed.{u} ⥤ TopCat.Pointed.{u} where
  obj a := ⟨TopCat.mappingCone.obj (forgetArrow.obj a), (TopCat.mappingCone.codomIncl.app (forgetArrow.obj a)) a.right.basepoint⟩
...
/-- The natural isomorphism `(X ∪ CY)/X ≅ SY`. -/
def codomPairQuotientIso : codomPair ⋙ TopPair.Pointed.forget ⋙ quotient ≅ Arrow.leftFunc ⋙ TopCat.Pointed.forget ⋙ suspension := sorry
...
-/
noncomputable def forgetConePairIso : MorphismProperty.Arrow.forget _ _ _ ⋙ TopCat.Pointed.mappingCone.conePair ⋙ TopPair.Pointed.forget ≅ TopPair.Pointed.forget ⋙ MorphismProperty.Arrow.forget _ _ _ ⋙ TopCat.mappingCone.conePair := NatIso.ofComponents (fun Xₚ ↦ { hom := 𝟙 _, inv := 𝟙 _ })
...
/-- The first space of the pair with adjoined baspoints is the same as adjoining a basepoint to the first space. -/
noncomputable def proj₁Iso : adjoinPoint ⋙ TopPair.Pointed.proj₁ ≅ proj₁ ⋙ TopCat.adjoinPoint := Iso.refl _

/-- The second space of the pair with adjoined baspoints is the same as adjoining a basepoint to the second space. -/
noncomputable def proj₂Iso : adjoinPoint ⋙ TopPair.Pointed.proj₂ ≅ proj₂ ⋙ TopCat.adjoinPoint := Iso.refl _

/-- The quotient of a topological pair with adjoined basepoints is isomorphic to the same pair without adjoining basepoints. -/
def quotientIso : TopPair.adjoinPoint ⋙ TopPair.Pointed.forget ⋙ TopCat.quotient ≅ TopCat.quotient := sorry -- TODO: data

end adjoinPoint

/-- The pairwise suspension of topological spaces. See `TopCat.suspension` for what the suspension is on `TopCat`. -/
...
/-- The quotient of the the pairwise suspension is isomorphic to the reduced suspension of the quotient. -/
def quotientIso : TopPair.suspension ⋙ TopPair.Pointed.forget ⋙ TopCat.quotient ≅ TopCat.quotient ⋙ TopCat.Pointed.suspension.reduced := sorry -- TODO: data

/-- The quotient of the the `n`-fold pairwise suspension is isomorphic to the `n`-fold reduced suspension of the quotient. -/
noncomputable def quotientIso' : (n : ℕ) → (suspension' n) ⋙ TopCat.quotient ≅ TopCat.quotient ⋙ TopCat.Pointed.suspension.reduced' n
  | 0 => Iso.refl _
  | n + 1 => Functor.associator _ _ _ ≪≫ Functor.isoWhiskerLeft _ quotientIso ≪≫ (Functor.associator _ _ _).symm ≪≫ Functor.isoWhiskerRight (quotientIso' n) _
...
/-- The first space of a pairwise suspension of a pointed pair is the same as the suspension of the second space. -/
noncomputable def proj₁Iso : TopPair.Pointed.forget ⋙ TopPair.suspension ⋙ Pointed.proj₁ ≅ Pointed.proj₁ ⋙ TopCat.Pointed.forget ⋙ TopCat.suspension := Functor.isoWhiskerLeft _ TopPair.suspension.proj₁Iso ≪≫ (Functor.associator _ _ _).symm ≪≫ Functor.isoWhiskerRight Pointed.proj₁ForgetIso.symm _

/-- The first space of a pairwise suspension of a pointed pair is the same as the `n`-fold suspension of the second space. -/
noncomputable def proj₁Iso' : (n : ℕ) → suspension' n ⋙ Pointed.proj₁ ≅ Pointed.proj₁ ⋙ TopCat.Pointed.suspension' n
  | 0 => Iso.refl _
  | n + 1 => Functor.associator _ _ _ ≪≫ Functor.isoWhiskerLeft _ proj₁Iso ≪≫ (Functor.associator _ _ _).symm ≪≫ Functor.isoWhiskerRight (proj₁Iso' n) _

/-- The second space of a pairwise suspension of a pointed pair is the same as the suspension of the second space. -/
noncomputable def proj₂Iso : TopPair.Pointed.forget ⋙ TopPair.suspension ⋙ Pointed.proj₂ ≅ Pointed.proj₂ ⋙ TopCat.Pointed.forget ⋙ TopCat.suspension := Functor.isoWhiskerLeft _ TopPair.suspension.proj₂Iso ≪≫ (Functor.associator _ _ _).symm ≪≫ Functor.isoWhiskerRight Pointed.proj₂ForgetIso.symm _

/-- The second space of a pairwise suspension of a pointed pair is the same as the `n`-fold suspension of the second space. -/
noncomputable def proj₂Iso' : (n : ℕ) → suspension' n ⋙ Pointed.proj₂ ≅ Pointed.proj₂ ⋙ TopCat.Pointed.suspension' n
  | 0 => Iso.refl _
  | n + 1 => Functor.associator _ _ _ ≪≫ Functor.isoWhiskerLeft _ proj₂Iso ≪≫ (Functor.associator _ _ _).symm ≪≫ Functor.isoWhiskerRight (proj₂Iso' n) _
